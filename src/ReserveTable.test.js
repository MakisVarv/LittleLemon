...
describe('ReserveTable Component', () => {
  ...
  test('renders all form fields', () => {
    ...
    expect(
      screen.getByLabelText(/Date of Reservation/i),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText(/Time of Reservation/i),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText(/Number of Diners/i),
    ).toBeInTheDocument();
  ...
  test('shows error when submitting empty form', () => {
    ...
    expect(
      screen.getByText(
        /First name must be between 2 and 30 characters/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /Last name must be between 2 and 30 characters/i,
      ),
    ).toBeInTheDocument();
    expect(screen.getByText(/Date is required/i)).toBeInTheDocument();
    expect(screen.getByText(/Time is required/i)).toBeInTheDocument();
    expect(
      screen.getByText(/Occasion is required/i),
    ).toBeInTheDocument();
  });
  ...
  test('removes booked time from availableTimes after submission', () => {
    ...
    expect(
      screen.getByRole('option', { name: '18:00' }),
    ).toBeInTheDocument();
